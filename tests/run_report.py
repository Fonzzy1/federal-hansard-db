...
def get_all_test_files():
    ...
    return sorted(
        f
        for f in TESTS_DIR.glob("*.xml")
        if f.stem != "test" and not f.stem.endswith(".xml")
    )
    ...
    for test_file in test_files:
        ...
        try:
            ...
            if not isinstance(parsed_result, list):
                report["error"] = (
                    f"parse() returned {type(parsed_result).__name__}, expected list"
                )
                results.append(report)
                continue
...
def print_report(results):
    ...
    for r in valid_results:
        for name in COUNT_METRICS:
            ...
            if metric_result and hasattr(metric_result, "count"):
                col_widths[name] = max(
                    col_widths[name], len(str(metric_result.count))
                )
    ...
    for r in valid_results:
        ...
        row_parts.append(
            f"{r.get('parser', '').replace('parsers.', ''):<{col_widths['Parser']}}"
        )
        for name in COUNT_METRICS:
            metric_result = r.get("metrics", {}).get(name)
    ...
    if not active_issues:
        ...
        for r in valid_results:
            for name in active_issues:
                metric_result = r.get("metrics", {}).get(name)
                if metric_result and hasattr(metric_result, "count"):
                    issue_col_widths[name] = max(
                        issue_col_widths[name], len(str(metric_result.count))
                    )
    ...
    for r in valid_results:
        ...
        for name in ISSUE_METRICS:
            ...
            if metric_result.examples:
                for ex in metric_result.examples:
                    print(f"      - {ex}")
